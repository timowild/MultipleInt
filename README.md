...
MultipleInt is a header-only C++20 library which takes over these bit-shifting operations so that multiple integers can be encoded from and decoded into a single integer variable. In addition, several operations can be applied on these packed integers (called `multiple_int`s) which consist in:
